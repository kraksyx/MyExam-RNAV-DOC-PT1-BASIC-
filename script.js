...
function finishTest() {
  ...
  questions.forEach((q, i) => {
    ...
    qDiv.innerHTML = `<strong>Q${i + 1}</strong>: ${q.question}<br>Your answer: ${q.options[userAnswer] || "No answer"}<br>Correct answer: ${q.options[q.correct]}<br><br>`;
    summary.appendChild(qDiv);
  });

  const retryButtons = `
    <div class="controls" style="justify-content: center;"><button onclick="retryAll()">Retry All</button><button onclick="retryIncorrect()">Retry Incorrect</button><button onclick="returnToMenu()">Menu</button></div>
  `;

  summary.innerHTML = `<h2>Test Summary</h2><p>You got ${correctCount} out of ${questions.length} correct.</p>${retryButtons}<br><br>` + summary.innerHTML;
...
function retryIncorrect() {
  ...
  userAnswers = Array(newQuestions.length).fill(undefined);
  lockedAnswers = Array(newQuestions.length).fill(false);
  currentQuestionIndex = 0;
  document.querySelector(".container").style.display = "flex";
  document.getElementById("summary-container").style.display = "none";
  renderNav();
...
function startExam() {
  ...
  questions.forEach(q => {
    let correctAnswer = q.options[q.correct];
